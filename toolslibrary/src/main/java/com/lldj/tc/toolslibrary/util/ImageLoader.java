...
//                    System.out.println("不进行网络请求了");
                ...
                try {
                    ...
                    if ("https".equals(mUrl.getProtocol())) {// ssl证书管理在这里
                        HttpsURLConnection connHttps = (HttpsURLConnection) mUrl.openConnection();
                        connHttps.setSSLSocketFactory(HttpTool.socketFactory);
                        connHttps.setHostnameVerifier(HttpTool.verifier);
                        conn = connHttps;
                    } else {
